...
export default class RowiElement extends HTMLElement {
    ...
    constructor() {
        ...
        Object.defineProperties(this, Object.entries(props).reduce(
            (accum, [key, prop]: [string, Prop]) => {
                let attr = prop.attr || this.#getDefaulAttr(key)
                this.#attrs[attr] = {name: key, prop}
                if (prop.handler != null) {
                    this.#handlers[key] = (ev: CustomEvent) => prop.handler.apply(this, [ev.detail])
    ...
    #setInitialValues(propSetting=undefined) {
            ...
            if (prop.type != 'boolean') {
                try {
                    ...
                    if (prop.validator) {
                        ...
                        if (!valid)
                            throw `Prop "${name}" value ${propValue} is not valid`
                    }
                } catch (error) {
                    console.error(error)
    ...
    $set(propName, value, preventEvent = true) {
        ...
        if (preventEvent && handler != null)
            this.removeEventListener('$' + propName, handler)
        this[propName] = value
        if (preventEvent && handler != null)
            setTimeout(() => this.addEventListener('$' + propName, handler))
    }
    ...
    $createElement(elem, refs) {
        refs = refs || {}
        let tag: string | Element | DocumentFragment = 'div'
        let opts = {}
        if (
